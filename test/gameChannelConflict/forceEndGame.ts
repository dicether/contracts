...
contract("GameChannelConflict-ForceEnd", (accounts) => {
    ...
    describe("serverForceGameEnd", async () => {
        ...
        it("Should fail if time span too low", async () => {
            ...
            return expect(gameChannel.serverForceGameEnd(user, d.gameId, {from: server})).to.be.rejectedWith(
                TRANSACTION_ERROR
            );
        });
        ...
        it("Should fail if wrong sender", async () => {
            ...
            return expect(gameChannel.serverForceGameEnd(user, d.gameId, {from: user})).to.be.rejectedWith(
                TRANSACTION_ERROR
            );
        });
        ...
        it("Force end should succeed", async () => {
            ...
            const regularNewBalance = new BN(
                fromGweiToWei(
                    calcNewBalance(
                        d.gameType,
                        d.num,
                    ...
                    const newBalance = max(
                        d.balance
                            .add(
                                new BN(
                                    fromGweiToWei(
                                        calcMaxUserProfit(d.gameType, d.num, fromWeiToGwei(d.value.toString()))
                                    )
                                )
                            )
                            .add(NOT_ENDED_FINE),
